...
export default function Home({ districtName }) {
  ...
  const casteVote = async (_candidateID: string, _position?: string) => {
    try {
      ...
      await ElectionSmartContract.methods.vote(_candidateID, electionAddress).send({ from: loggedInAccountAddress });
  ...
  console.log({ currentElection, leadingCandidate, districtName })
  return (
    <div>
      <Head>
